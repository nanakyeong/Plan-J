...
public class PopupDialog extends JDialog {
    ...
    public PopupDialog(JFrame parent, JPanel dayPanel) {
        ...
        // map.html 파일을 로드할 JEditorPane을 생성
        JEditorPane mapPane = new JEditorPane();
        mapPane.setEditable(false);

        // map.html 파일을 로드합니다.
        try {
            URL mapUrl = getClass().getResource("/map.html"); // 리소스 폴더에 있는 map.html 파일
            mapPane.setPage(mapUrl);
        } catch (Exception e) {
            e.printStackTrace();
            mapPane.setContentType("text/html");
            mapPane.setText("<html>지도를 불러올 수 없습니다.</html>");
        }

        // mapPane을 JScrollPane으로 감싸기
        JScrollPane mapScrollPane = new JScrollPane(mapPane);
        mapScrollPane.setPreferredSize(new Dimension(294, 400)); // 원하는 크기로 설정

        // 오른쪽에 mapScrollPane 추가하기 위해 rightPanel 사용
        ...
        placeList.addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (selectedItem != null) {
                    ...
                    if (contentId != null) {
                        String details = fetchDetailInfo(contentId, contentTypeId);
